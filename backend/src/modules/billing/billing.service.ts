...
export class BillingService {
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      let customerId = await this.getOrCreateStripeCustomer(companyId);

      const frontendUrl = this.configService.get<string>('FRONTEND_URL') || 'http://localhost:3100';
      const successUrl = createCheckoutDto.successUrl || `${frontendUrl}/billing/success?session_id={CHECKOUT_SESSION_ID}`;
      const cancelUrl = createCheckoutDto.cancelUrl || `${frontendUrl}/billing/cancel`;
  ...
  async getPlans(): Promise<any[]> {
    try {
      ...
      throw new InternalServerErrorException(`Failed to fetch plans: ${error.message}`);
    }
  }
   ...
   */
  async getSubscription(companyId: string): Promise<SubscriptionDocument | null> {
  ...
  async updateUsage(
    ...
    try {
      ...
      await (subscriptionItem as any).createUsageRecord(updateUsageDto.subscriptionItemId, {
      ...
      await this.auditService.createLog({
        ...
        resource: AuditResource.SETTINGS,
        resourceId: (subscription._id as any)?.toString() || String(subscription._id),
        userId,
        companyId,
  ...
  private async handleCheckoutSessionCompleted(session: Stripe.Checkout.Session): Promise<void> {
    ...
    console.log('Checkout session completed', { sessionId: session.id, companyId });
  }
