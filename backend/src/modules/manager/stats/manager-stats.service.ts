...
export class ManagerStatsService {
  ...
  async getOverview(companyId: string, departmentId?: string): Promise<any> {
    ...
    const activeProjects = await this.projectModel.countDocuments(projectQuery).exec();
  ...
  async getTaskStats(companyId: string, departmentId?: string): Promise<any> {
    ...
    const weeklyCompleted = recentTasks.filter(t => t.status === TaskStatus.DONE).length;
  ...
  async getTeamStats(companyId: string, departmentId?: string): Promise<any> {
    ...
    const totalTeamTasks = teamProductivity.reduce((sum, member) => sum + member.totalTasks, 0);
    const totalCompleted = teamProductivity.reduce((sum, member) => sum + member.completedTasks, 0);
    const averageCompletionRate = teamProductivity.length > 0
      ? Math.round(teamProductivity.reduce((sum, member) => sum + member.completionRate, 0) / teamProductivity.length)
      : 0;
