...
export class ManagerProjectsService {
   ...
   */
  async getDepartmentProjects(companyId: string, departmentId?: string): Promise<any[]> {
    const query: any = {
   ...
   * Get project details
   */
  async getProjectDetails(projectId: string, companyId: string, departmentId?: string): Promise<any> {
    ...
    if (departmentId) {
      ...
      if (department) {
        const hasAccess = project.assignedUsers?.some((userId: any) =>
          department.members.some(memberId => memberId.toString() === userId._id?.toString() || userId.toString())
        );
        if (!hasAccess) {
   ...
   */
  async getProjectTasks(projectId: string, companyId: string, departmentId?: string): Promise<any[]> {
