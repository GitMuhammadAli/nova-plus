...
import { Plus, Search, Edit, Trash2, Users, UserCog, Loader2, RefreshCw, Building2, TrendingUp } from "lucide-react";
...
export function DepartmentsClient({ initialDepartments, initialUsers }: DepartmentsClientProps) {
  ...
  useEffect(() => {
    if (initialDepartments.length > 0 && departments.length === 0) {
      dispatch({ type: 'departments/fetchDepartments/fulfilled', payload: initialDepartments });
    }
    if (initialUsers.length > 0 && users.length === 0) {
      dispatch({ type: 'users/fetchCompanyUsers/fulfilled', payload: initialUsers });
    }
  }, [initialDepartments, initialUsers, departments.length, users.length, dispatch]);

  const allDepartments = departments.length > 0 ? departments : initialDepartments;
  ...
  const totalMembers = allDepartments.reduce((sum, d) => sum + (d.members?.length || 0), 0);
  const departmentsWithManagers = allDepartments.filter((d) => d.managerId).length;
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold mt-1 text-green-600">{activeDepartments}</p>
              </div>
            </div>
          </Card>
        </motion.div>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold mt-1">{departmentsWithManagers}</p>
              </div>
            </div>
          </Card>
        </motion.div>
              ...
              {formData.memberIds.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.memberIds.map((memberId) => {
                    const member = Array.isArray(allUsers) ? allUsers.find((u) => (u._id || u.id) === memberId) : null;
                    return (
